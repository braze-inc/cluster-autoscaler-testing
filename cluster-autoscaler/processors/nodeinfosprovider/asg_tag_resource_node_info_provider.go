...
func (p *AsgTagResourceNodeInfoProvider) Process(ctx *context.AutoscalingContext, nodes []*apiv1.Node, daemonsets []*appsv1.DaemonSet, taintConfig taints.TaintConfig, currentTime time.Time, selector labels.Selector) (map[string]*schedulerframework.NodeInfo, errors.AutoscalerError) {
	nodeInfos, err := p.mixedTemplateNodeInfoProvider.Process(ctx, nodes, daemonsets, taintConfig, currentTime, selector)
	if err != nil {
		return nil, err
	}
